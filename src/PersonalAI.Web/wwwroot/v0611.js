(() => {
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (method === "POST" && isChatUrl(url) && typeof init.body === "string") {
      try {
        ...
        // Existing validation handles malformed chat payloads.
      }
    }
  ...
  function initialize() {
    ...
    if (messages) {
      // Only watch direct message rows. Watching the whole subtree caused v0.6.1
      // to observe the suggestion card that it created itself and loop forever.
  ...
  function hydrateSuggestions() {
    ...
    try {
      ...
      const activeIds = new Set(activeSuggestions.map(item => item.id));

      document.querySelectorAll("#messages .memory-suggestion").forEach(node => {
        if (!activeIds.has(node.dataset.memorySuggestionId)) node.remove();
      });

      activeSuggestions.forEach(suggestion => {
        if (document.querySelector(
          `.memory-suggestion[data-memory-suggestion-id="${cssEscape(suggestion.id)}"]`)) {
          return;
        }
  ...
  async function persistSuggestionAfterSuccess(context) {
    ...
    if (suggestions.some(item =>
      ...
      && normalize(item.content) === normalize(context.candidate.content))) return;
  ...
  function pruneSuggestionStore() {
    ...
    writeSuggestions(readSuggestions().filter(item =>
      validIds.has(item.conversationId)
    ...
    try {
      ...
      if (card) {
        ...
        saved.textContent = response.ok
          ? "✓ Đã lưu vào trí nhớ"
          : "✓ Nội dung này đã có trong trí nhớ";
  ...
  function updateVersionLabels() {
    ...
    if (memoryHint) {
      memoryHint.textContent = "Có thể gợi ý trí nhớ từ câu nói rõ ràng; chỉ lưu khi bạn xác nhận.";
    }
  }

  function injectStyles() {
    if (document.querySelector("#v0611Styles")) return;
    const style = document.createElement("style");
    style.id = "v0611Styles";
    style.textContent = `
