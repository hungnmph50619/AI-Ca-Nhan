(() => {
  ...
  function ensureTaskDialog() {
    ...
    introTitle.textContent = "Tác vụ có Undo Foundation v1.7.0";
    ...
    note.textContent = "v1.7.0 giữ tối đa 50 tác vụ trong mỗi không gian trên máy, hỗ trợ phụ thuộc giữa tác vụ và giữa các bước. Phụ thuộc không làm tăng quyền tự động: mỗi bước vẫn phải được người dùng chủ động chạy và các thao tác có tác động vẫn cần xác nhận." ;
  ...
  function formatArguments(toolName, args) {
    ...
    switch (toolName) {
      ...
      case "browser.session.info":
        break;
      case "browser.navigate":
        add("URL", value.url);
        break;
      case "browser.page.observe":
        add("Ký tự tối đa", value.maximumCharacters);
        add("Link tối đa", value.maximumLinks);
        break;
